...
class Order(models.Model):
    ...
    table_number: int = models.PositiveIntegerField(verbose_name="table number")
    dishes: List[dict] = models.JSONField(verbose_name="list of dishes", default=list)
    ...
    def count_total_price(self) -> None:
        ...
        self.total_price = sum(
            dish.get("price", 0) for dish in self.dishes if isinstance(dish, dict)
        )
        ...
        if isinstance(self.total_price, str):
            try:
                ...
                raise ValidationError(
                    "Invalid format for 'total_price'.\
                                      Must be a number."
                )

        if not isinstance(self.dishes, list):
            raise ValidationError("The 'dishes' field must be a list of dictionaries.")

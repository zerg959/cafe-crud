...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def get_order_by_table(self, request):
        ...
        if table_number is None or int(table_number) <= 0:
            return Response(
                {"error": "Enter positive table number."}, status=400)
        try:
            table_number = int(table_number)
        except (ValueError, TypeError):
            return Response(
                {"error": "Invalid input: Table number must be an integer."},
                status=400)
    ...
    def calculate_revenue(self, request):
        ...
        total_revenue = paid_orders.annotate(
            numeric_total_price=Cast('total_price', output_field=FloatField())
        ).aggregate(total=Sum('numeric_total_price'))['total'] or 0
